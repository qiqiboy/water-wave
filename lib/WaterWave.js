    ...
    function Water() {
        ...
        return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Water.__proto__ || Object.getPrototypeOf(Water)).call.apply(_ref, [this].concat(args))), _this), _this.createWave = function (ev) {
            ...
            if (!disabled) {
                ...
                var maxRadius = typeof radius === 'number' ? radius : Math.sqrt(Math.pow(Math.max(pointX, width - pointX), 2) + Math.pow(Math.max(pointY, height - pointY), 2));
                ...
                var run = function run() {
                    ...
                    var ratio = Math.min(1, Tween(offset, 0, 1, duration));
                    var opacity = press === 'down' && _this.startState ? alpha : Math.min(alpha, alpha - (ratio - .7) * alpha / .3);
