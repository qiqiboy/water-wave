    ...
    _createClass(Ripple, [{
        ...
        value: function render() {
            ...
            if (effect === 'wave') {
                ...
                var arcSize = ratio * maxRadius;

                if (!this.petalNumber) {
                    this.petalNumber = parseInt(Math.random() * 7) + 4; //随机出花瓣数量
                }

                var _angle = Math.PI * 2 / this.petalNumber; //每个花瓣的角度大小
                var x2 = Math.sin(_angle) * arcSize;
                var y2 = -Math.cos(_angle) * arcSize;
                ...
                var _arcSize = ratio * Math.min(width, height);
                ...
                var _angle2 = Math.PI * 2 / this.stars.length; //每个星星的角度大小
